...
export function calculateBugScore({ impact, priority, ticketCount = 0 }) {
  if (!impact || !priority) return "N/A";
  const levelConst = toUpperSnakeCase(impact);
  const priorityConst = toUpperSnakeCase(priority);

  const initialScore = BUG_SCORES[levelConst]?.[priorityConst];
  console.log("initialScore is %s", initialScore);

  if (!initialScore) return "N/A";
